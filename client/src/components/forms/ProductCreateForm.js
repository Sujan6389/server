...
const ProductCreateForm = ({handleSubmit,handleChange,values,setValues}) =>

    {
     //destructure
    const {title,description, price,categories,category,shipping,quantity,images,colors,brands,color,brand}
    =values;
    ...
    <form onSubmit={handleSubmit}>
...

             {
                 categories.length > 0 &&
                  categories.map((c)=>(
                      <option key={c.data} value={c._id}>{c.name}</option>
                  )
                  )
             }
